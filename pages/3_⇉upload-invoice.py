...
colored_header(
    label="Welcome to  Q&A Bot for PNG",
    description="Upload a PNG and ask questions about it, or if you have already uploaded a PNG, ask questions about it.",
...
def get_png_text(png_path):
 documents=[]
 for png in png_path:
   
    with NamedTemporaryFile(delete=False) as tmp:
            shutil.copyfileobj(png, tmp)
            tmp_path = Path(tmp.name)

    loader =UnstructuredImageLoader(file_path=str(tmp_path))
...
def vector_db_pdf(png_path) -> None:
    """
    ...
    #print(persist_directory)
        documents=get_png_text(png_path)
...
with st.sidebar:
    ...
    png_path= st.file_uploader("**Upload PNG**", type="png" , accept_multiple_files=True)
    ...
    if st.button("Set Params"):
        with st.spinner(text="Setting Params.."):
            ...
            st.session_state["pdf_qa_model"].vectordb = vector_db_pdf(png_path)
    ...
    if st.button("Upload and Set Params") and png_path is not None:
        with st.spinner(text="Uploading Png and Generating Embeddings.."):
            st.session_state["pdf_qa_model"].config = {
                "embedding": emb,
                "llm": llm,
                "load_in_8bit": load_in_8bit
            ...
            st.sidebar.success("Png uploaded and parameter generated successfully")
